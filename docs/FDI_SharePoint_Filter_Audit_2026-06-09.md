...
Objetivo: reducir el riesgo de `list view threshold` en la lista SharePoint `Cotizaciones 2026` sin cambiar `Patch`, estados ni logica de negocio.
...
- La formula critica era `CotizacionesGallery.Items` en `scrMisCotizaciones`: consultaba `Cotizaciones 2026` con filtros opcionales, por lo que con busqueda, vendedor y estado vacios podia intentar evaluar la lista completa.
...
Riesgo: si `q`, `idsVendSel` y `estadosSel` estaban vacios, la formula no tenia filtro obligatorio inicial sobre `Cotizaciones 2026`. Esto podia disparar errores por `list view threshold` en SharePoint.
...
| --- | --- | --- | --- | --- |
| `scrMisCotizaciones` | `CotizacionesGallery` | `Items` | `Cotizaciones 2026` | Corregida. Era el principal riesgo de threshold. |
| `scrGenerarPedido` | `ComboboxCanvas2` | `Items` | `Cotizaciones 2026` | Aceptable: filtra por `Estado.Value = "Comprada"`. |
| `scrFDI` | `scrFDI` | `OnVisible` | `Cotizaciones 2026` | Aceptable: `LookUp` por `Estado = "Borrador"` y usuario creador. |
| `scrFDI` | `SistemasButton` | `OnSelect` | `Cotizaciones 2026` | No se toca por alcance: contiene `Patch`/flujo de guardado. |
| `scrFDI` | `ContactoLookUpComboBox` | `Items/SearchItems` | `Contactos`, `Clientes`, `Cotizaciones 2026` | Usa `Choices`/lookup; sin lectura masiva directa de cotizaciones. |
| `scrGenerarFolio` | `ContactoLookUpComboBox2` | `Items` | `Contactos`, `Clientes`, `Cotizaciones 2026` | Usa `Choices`/lookup; sin lectura masiva directa de cotizaciones. |
...
| `scrContactosClientes` | `GalleryDirectorio` | `Items` | `Clientes`, `Contactos` | Usa `Search(Filter(...Activo=true), q, Title)`; no afecta `Cotizaciones 2026`, pero conviene migrar a `StartsWith` si esas listas crecen. |
...
P0:
- Mantener el filtro base por `Created >= Date(2026, 1, 1)` en cualquier galeria principal sobre `Cotizaciones 2026`.
