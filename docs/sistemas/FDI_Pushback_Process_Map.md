...
| `T` Sección: Tarima | Tarimas | Tabla editable | Método = `Diseño` | `PayloadSistemaJson.Tarimas` | `colListadoTarimas` | Al menos detalle si se agregan renglones. | Tabla compartida filtrada por `SistemaId`. |
...
| `CLI` Adjuntar planos/diseño de cliente | Planos/diseño de cliente | Adjuntos existentes | Método = `Planos/diseño de cliente` | Adjuntos de `Cotizaciones 2026`; payload indicador | N/A | Archivo recomendado. | El cliente provee sus planos; se apoya en adjuntos del formulario principal. |
