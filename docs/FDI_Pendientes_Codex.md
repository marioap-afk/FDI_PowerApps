...
| --- | --- | --- | --- | --- |
| 0c | **BLOQUEANTE — campos requeridos vacíos → Enviar no valida**: `Correo empresarial` y `Número de teléfono` (`Required`) quedan vacíos porque el autollenado del contacto se rompe: `ContactoLookUpComboBox.OnChange` usa `Selected.Id` (minúscula) → **null** → `LookUp(Contactos, ID=null)` → 0 filas. El Monitor lo confirma (`SubmitForm` validationErrors). Mismo mecanismo deja **display fields vacíos** al editar borradores. Fix: Codex corrige la resolución del ID del contacto (`.Selected.ID` o `LookUp(Contactos, Title=Selected.Value).ID`) y audita convenciones `.Selected.ID`/`.Id` + hidratación de tarjetas lookup. | 🔴 **Bloqueante** | [FDI_Form_CamposVacios_Bug_Report.md](FDI_Form_CamposVacios_Bug_Report.md) | ❌ abierto |
