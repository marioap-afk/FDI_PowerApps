...
| --- | --- | --- | --- | --- |
| 0i | **BLOQUEANTE — el reskin no abre en Studio: `PA2108 Unknown property 'Radius*' for control type 'Label@2.5.1'`**. El reskin aplicó `RadiusTopLeft/TopRight/BottomLeft/BottomRight` a controles `Label@2.5.1` (no soportan radio por esquina): **192 líneas / ~48 Labels** en `scrFDI`(36), `scrFDI_MZL`(32), `scrFDI_DIN`/`_DRV`/`_PBK`/`_SEL`(28 c/u), `scrFDI_CAN`/`_CFL`/`_MEZ`(4 c/u). Quitar los `Radius*` de los Label (o moverlos al `GroupContainer`/`Button` que los envuelve). `GroupContainer`(494)/`Button`(276) **son válidos**; verificar `Image@2.2.3`(20). Entregar `.msapp` **exportado desde Studio** (no `pac canvas pack`: deja rutas con `/` y no valida propiedades → mi gate no lo atrapa). | 🔴 **Bloqueante** | error al abrir en Studio (PA2108, sesión 9af22165…) | ❌ abierto (Codex corrigiendo) |
| 0h | **Flujo Power Automate — error en el Sistema "Otro" (OT)**: el flujo (Creación FDI / Office Script) **sigue fallando** para el sistema OT. **Falta el detalle exacto** (mensaje + acción que falla). Hipótesis a verificar: OT no tiene tabla/lista normalizada como los demás sistemas → la acción de tabla / `GetItems` falla (patrón 404 *"List/Table not found"* ya visto en 0b). | 🔴 Alta | ⏳ pendiente de trace/mensaje del usuario | ❌ abierto |
