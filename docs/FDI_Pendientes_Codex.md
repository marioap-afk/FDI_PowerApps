...
- **Empaquetado BLOQUEADO** por **0i** (reskin no abre en Studio, PA2108). Último build empaquetable
  sano = **`a97e7eb` (1.0.0.17, pre-reskin)** — regenerable con `git checkout a97e7eb` + packer.
- Builds de reskin **rotos**: `2ee102a` (1.0.0.18) y `6ed8d89` (1.0.0.19) → **no desplegar**.
...
| --- | --- | --- | --- | --- |
| 0i | **BLOQUEANTE — el reskin no abre en Studio: `PA2108 Unknown property 'Radius*' for control type 'Label@2.5.1'`**. El reskin aplicó `RadiusTopLeft/TopRight/BottomLeft/BottomRight` a controles `Label@2.5.1` (no soportan radio por esquina). **Intento `6ed8d89` (1.0.0.19) incompleto**: quitó solo las **8 líneas** que Studio mostró en el lote (Studio reporta por lotes); **quedan 184 líneas Label** en `scrFDI_MZL`(32), `scrFDI`(28), `scrFDI_PBK`(28), `scrFDI_DIN`(28), `scrFDI_SEL`(28), `scrFDI_DRV`(28), `scrFDI_CFL`(4), `scrFDI_MEZ`(4), `scrFDI_CAN`(4). **Hay que barrer TODOS los Label** (no solo los del mensaje) y quitar/mover el `Radius*`. `GroupContainer`(494)/`Button`(276) **son válidos**; verificar `Image@2.2.3`(20). **Entregar `.msapp` exportado desde Studio** (debe abrir sin PA2108): el binario sigue siendo `pac canvas pack` (rutas con `/`), que no valida propiedades → mi gate no lo atrapa; un export real de Studio usa `\`. | 🔴 **Bloqueante** | error al abrir en Studio (PA2108, sesión 9af22165…) | ❌ abierto (fix `6ed8d89` incompleto: 184/192 restantes) |
