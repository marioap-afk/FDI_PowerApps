...
| --- | --- | --- | --- | --- |
| 0d | **BLOQUEANTE — el proceso de cotización (Enviar) no completa aunque el form guarde**: `CotizaciónForm.OnSuccess` es un bloque único de ~63 escrituras sobre 16 listas (`Sistemas por cotización` + 7 `Hija *` + 9 `Sistema *`) y `Navigate(scrCorreo)` es la **última** línea. Si **una** operación falla, aborta antes de navegar → cotización a medias. En el Monitor: `PATCH 200` del form pero **0 escrituras a esas listas y sin `Navigate`**. Falta **Monitor nuevo** del intento actual para señalar la lista/columna que rompe; luego Codex la corrige y **endurece el OnSuccess** (que `Navigate` no quede rehén de las 63 escrituras). | 🔴 **Bloqueante** | [FDI_Enviar_OnSuccess_Bug_Report.md](FDI_Enviar_OnSuccess_Bug_Report.md) | ❌ abierto (falta trace) |
